...
class ITjuziSpider(RedisCrawlSpider):
    ...
    def parse_item(self, response):
        ...
        if cpy1:
            item['company_name'] = cpy1.find('div', class_='line-title').find('h1',
                                                                              'seo-important-title').get_text().strip().replace(
                '\t', '').replace('\n', '')
        ...
        if cpy2:
            ...
            item['found_time'] = cpy2_content[3].contents[1].get_text().strip()[len('成立时间：'):] if cpy2_content[
                3] else ''
            item['company_size'] = cpy2_content[3].contents[3].get_text().strip()[len('公司规模：'):] if cpy2_content[
                3] else ''
